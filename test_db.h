...
void TestsMyCustom()
{
...

    {
        ...
        AssertEqual(pair<Date, string>({2019, 12, 3},"f"), db.Last({2019, 12, 4}), " My test 6");
        ...
        db.Add({2019, 12, 3}, "m");
        AssertEqual(pair<Date, string>({2019, 12, 3}, "m"), db.Last({2019, 12, 3}), " My test 9");
...

    {
        ...
        AssertEqual(pair<Date, string>({2019, 12, 1}, "c"), db.Last({2019, 12, 1}), " My test 20");

        db.Add({2019, 12, 1}, "b");
        AssertEqual(pair<Date, string>({2019, 12, 1}, "c"), db.Last({2019, 12, 1}), " My test 21");
...
void TestDatabase() {
  ...
  // Last
  {
    ...
    db.Add(d1, "e1");
    db.Add(d2, "e2");
    AssertEqual(db.Last(d), pair<Date, string>(d2, "e2"), "Db Last 1");
    Date d3(2018, 12, 24);
    db.Add(d3, "e3");
    AssertEqual(db.Last(d), pair<Date, string>(d3, "e3"), "Db Last 2");
    ...
    AssertEqual(db.Last(d), pair<Date, string>(d2, "e2"), "Db Last 4");

    AssertEqual(db.Last(d1), pair<Date, string>(d1, "e1"), "Db Last 5");
    db.Add(d2, "e4");
    AssertEqual(db.Last(d2), pair<Date, string>(d2, "e4"), "Db Last 6");
  }
